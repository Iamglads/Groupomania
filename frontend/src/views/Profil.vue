  ...
  <div class="profil">
    <card class="profil__content--info">
      <md-card class="info-bloc">
        ...
        <md-card-header>
          <h1 class="name">
            {{ user.firstname }} <strong>{{ user.lastname }}</strong> <br />
            <span class="fonction">{{ user.fonction }}</span>
          </h1>
        </md-card-header>
    ...
    <div class="profil__content col-md-6 ">
          ...
          <div class="infos">
            <div>
              ...
              <form class="add-picture">
                ...
                <button class="btn btn-primary" :click="addPicture()">
                  Ajouter
                </button>
              </form>
      ...
      <section class="profil__content--form">
        <form v-if="editForm" id="contact-form" name="contact-form">
          ...
          <button
            @click="updateInfoAccount(user.id)"
            class="btn-update-profil col-sm-3"
          >
            Enregistrer
          </button>
        </form>
...
export default {
  ...
  methods: {
    ...
    addPicture() {
      ...
      axios
        .post("http://localhost:5000/api/user/picture", formdata, {
          headers: {
    ...
    updateInfoAccount(currentUserId) {
      ...
      axios
        .put(
          `http://localhost:5000/api/user/unique/${currentUserId}`,
          {
    ...
    deleteAccount() {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          axios
            .delete(`http://localhost:5000/api/user/unique/${this.user.id}`, {
              headers: {
                Authorization: `Bearer ${this.user.token}`,
                Authorization: `Bearer ${this.currentUser.token}`,
                "Content-type": "application/json",
